...
//	*((u8 *)ctx->regs.entry_rip) = 0x55;//typecast the address to long type pointer,  dereference it to store the pushrbp operation code
	...
	//Manually implement PUSHRBP_OPCODE by pushing the function to the top of the stack.
...
int do_info_registers(struct exec_context *ctx, struct registers *regs)
{
	...
	// find the child process since parent is called just after breakpoint is encountered the registers in the child process remain as it is
        ...
        if(flag == 0)return -1;	//if child not found
	...
	regs->rbx = ch_reg.rbx;
	regs->rax = ch_reg.rax;
	regs->entry_rip = ch_reg.entry_rip;
...
int do_backtrace(struct exec_context *ctx, u64 bt_buf)
{
	...
	struct exec_context *child;
        ...
        if(flag == 0)return -1;
	unsigned long long *buf = (unsigned long long *)bt_buf;
	buf[0] = child->regs.entry_rip;
	int bt_count = 1;
	u64 temp = child->regs.entry_rsp;
	while(temp<child->regs.rbp){
		//if(*((u8 *)temp) == 0x55){// if the instruction refers to a function
			buf[bt_count++] = *((u32 *)temp);
		//}
		temp++;
	}
	return bt_count;
