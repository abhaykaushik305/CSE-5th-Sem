...
struct debug_info{
	...
	u32 num_bp;//store current number of breakpoints from 
	u32 bp_count;//store number of breakpoints created from the beginning
	u64 *bt_buf;//buffer to store the backtrace information once int3 hadler is called
	u32 bt_count;//store count of stack frames
};
